...
function SDoodStaking() {
  ...
  const sdoodContract = new ethers.Contract(
    config.SDOODADDRESS,
    SDOODABI,
    newSigner
  );
  ...
  console.log("sdoodStakingContract: ", sdoodStakingContract);
  console.log("sdoodContract: ", sdoodContract);
  ...
  async function claimState(account) {
    try {
      ...
      const c_stakedDay = Number(stakeInfo[2]);
      const c_lefttime = Number(stakeInfo[0]);
      const c_stakedAmount = Number(stakeInfo[1]) / 10 ** 18;
      setStakedAmount(c_stakedAmount);
      // eslint-disable-next-line no-shadow
      const stratTime = Date.now() / 1000; // use UNIX timestamp in seconds
      // eslint-disable-next-line no-shadow
      const endTime = c_lefttime + c_stakedDay; // use UNIX timestamp in seconds
      ...
      if (c_stakedDay === 1) {
        setEarnPercent(5);
      } else if (c_stakedDay === 2) {
        setEarnPercent(10);
      } else if (c_stakedDay === 3) {
        setEarnPercent(15);
      } else if (c_stakedDay === 4) {
        setEarnPercent(25);
      }
    } catch (error) {
      console.log(error);
  ...
  const staking = async (amount, day) => {
    if (amount <= 0) {
      ...
      await sdoodContract
        .approve(config.SDOODSTAKINGADDRESS, amountValue)
        .then((txApprove) => {
          txApprove.wait().then(() => {
            sdoodStakingContract
              .deposit(amountValue, ethers.BigNumber.from(day), {
  ...
  const claimStaking = async () => {
    ...
    await sdoodStakingContract.withdraw().then((tx) => {
      tx.wait().then(() => {
